...
public class Main {
    ...
    public static double cloudCounter = 0, rcloud = 0, creq = 0;
    public static double medgecounter = 0, medge = 0;
    public static double mfogcounter = 0, mfog = 0;
    public static double mcloudcounter = 0, mcloud = 0;
    public static double edgeCounter = 0, redge = 0, ereq = 0;
    public static double fogCounter = 0, rfog = 0, freq = 0;
...
//        cloud[6] = new CLOUD(6, 98, 90, 34, 1.8, "Underloaded", 10, 10);
    ...
    public static boolean edgeAllocation(String request)throws Exception{
        ...
        for(i=0;i<edge.length;i++){
            double temp = edge[i].freeRam - getRAM(request);
            double temp1 = edge[i].freeCPU - getCPU(request);
            double temp2 = edge[i].freeCapacity - getCapacity(request);
            if(!flag && temp>=0 && temp1>=0 && temp2>=0){
                id = i;
                difference = temp1;
                flag = true;
            }
            else if(flag){
                if(temp<difference && temp>=0 && temp1>=0 && temp2>=0){
                    id = i;
                    difference = temp1;
                }
            }
        ...
        if(!flag){
            ...
            edge[id].setReq(getId(request), getPriority(request), getStartTime(request), getDuration(request), getRAM(request), getCapacity(request), getCPU(request));
            System.out.println("Request_" + getId(request) + " is allocated to edgeNode_" + id);
            ereq++;
        }
        return flag;
    }
    ...
    public static boolean fogAllocation(String request) throws Exception {
        ...
        for(i=0;i<fogNode.length;i++){
            double temp = fogNode[i].freeRam - getRAM(request);
            double temp1 = fogNode[i].freeCPU - getCPU(request);
            double temp2 = fogNode[i].freeCapacity - getCapacity(request);
            if(!flag && temp>=0 && temp1>=0 && temp2>=0){
                id = i;
                difference = temp1;
                flag = true;
        ...
        if(!flag){
            ...
            fogNode[id].setReq(getId(request), getPriority(request), getStartTime(request), getDuration(request), getRAM(request), getCapacity(request),  getCPU(request));
            freq++;
            System.out.println("Request_" + getId(request) + " is allocated to fogNode_" + id);
        }
        return flag;
    }
    ...
    public static boolean cloudAllocation(String request)throws Exception{
        ...
        for(i=0;i<cloud.length;i++){
            double temp = cloud[i].freeRam - getRAM(request);
            double temp1 = cloud[i].freeCPU - getCPU(request);
            double temp2 = cloud[i].freeCapacity - getCapacity(request);
            if(!flag && temp>=0 && temp1>=0 && temp2>=0){
                id = i;
                difference = temp1;
                flag = true;
        ...
        if(!flag){
            ...
            cloud[id].setReq(getId(request), getPriority(request), getStartTime(request), getDuration(request), getRAM(request), getCapacity(request), getCPU(request));
            creq++;
            System.out.println("Request_" + getId(request) + " is allocated to cloudNode_" + id);
        }
        return flag;
    }
